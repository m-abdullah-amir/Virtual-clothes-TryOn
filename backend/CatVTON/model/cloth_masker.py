...
class AutoMasker:
    ...
    def cloth_agnostic_mask(
        ...
        **kwargs
        ...
        accessory_protect_area = part_mask_of((accessory_parts := ['Hat', 'Glove', 'Sunglasses', 'Left-shoe', 'Right-shoe', 'Socks']), schp_lip_mask, LIP_MAPPING) | \
        ...
        # Close small holes (like neck holes) without creating a massive convex hull
        close_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (35, 35))
        mask_area = cv2.morphologyEx((mask_area * 255).astype(np.uint8), cv2.MORPH_CLOSE, close_kernel) // 255
